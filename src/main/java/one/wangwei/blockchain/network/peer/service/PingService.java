...
import one.wangwei.blockchain.network.peer.network.message.KeepAlive;
import one.wangwei.blockchain.network.peer.network.message.ping.CancelPings;
...
public class PingService {
    ...
    private final Map<String, PingContext> currentPings = new HashMap<>();
        ...
        if (!currentPings.containsKey(config.getPeerName())) {
            if (incrementAutoPingCountAndCheckIfFullPing() && discoveryPingEnabled) {
                if (!currentPings.containsKey(config.getPeerName())) {
                    ...
                    log.info("Periodic discovery ping skipped since there exists a ping already");
                }
            } else {
                keepAlive();
            }
    ...
    public void ping(final CompletableFuture<Collection<String>> future) {
        ...
        if (pingContext == null) {
            ...
            log.info("Attaching to the already existing ping context");
        }
    ...
    public void handlePing(final InetSocketAddress bindAddress, final Connection connection, final Ping ping) {
        ...
        if (currentPings.containsKey(pingPeerName)) {
            log.info("Skipping ping of {} since it is already handled.", pingPeerName);
            return;
        }

        if (pingPeerName.equals(connection.getPeerName())) {
            log.info("Handling {} of initiator {} with ttl={}", ping, pingPeerName, ping.getTtl());
        } else {
            log.info("Handling {} of initiator {} and forwarder {} with ttl={} and hops={}", ping, pingPeerName,
        ...
        if (next != null) {
            for (Connection neighbour : connectionService.getConnections()) {
                if (!neighbour.equals(connection) && !neighbour.getPeerName().equals(ping.getPeerName())) {
                    log.info("Forwarding {} to {} for initiator {}", next, neighbour.getPeerName(),
                            ping.getPeerName());
                    neighbour.send(next);
                }
            }
        ...
        for (PingContext pingContext : currentPings.values()) {
            if (!pingContext.getPeerName().equals(connection.getPeerName())) {
                ...
                if (next != null) {
                    ...
                    log.info("{} sent to new connection {}", next, connection.getPeerName());
                }
            }
        }
    }
    ...
    public void handlePong(final Pong pong) {
        ...
        if (pingContext == null) {
            log.warn("No ping context is found for {} from {} for initiator {}", pong, pong.getPeerName(),
                    pingPeerName);
            return;
        }
        ...
        while (pingIt.hasNext()) {
            ...
            if (pingContext.removePong(disconnectedPeerName)) {
                ...
                if (pingOwnerConnection != null) {
                    log.info("Removed pong of {} in ping of {}. Forwarding {} to {}", disconnectedPeerName, pingPeerName,
                            cancelPongs, pingOwnerConnection.getPeerName());
                    pingOwnerConnection.send(cancelPongs);
                } else {
                    log.info("Removed pong of {} in ping of {}", disconnectedPeerName, pingPeerName);
                }
            }
            ...
            for (Pong pong : new ArrayList<>(pingContext.getPongs())) {
                if (pong.getSenderPeerName().equals(disconnectedPeerName)) {
                    ...
                    rePing = true;
                    log.info("Removed Pong of {} in ping of {} since it was sent by {}", pong.getPeerName(), pingPeerName,
                    ...
                    if (connection != null) {
                        ...
                        connection.send(msg);
                        log.info("Forwarded {} to {} for ping of {}", msg, connection.getPeerName(), pingPeerName);
                    }
                }
            }
                ...
                if (next != null) {
                    ...
                    for (Connection connection : connectionService.getConnections()) {
                        if (!(connection.getPeerName().equals(pingPeerName) || connection.getPeerName()
                            ...
                            connection.send(next);
                            log.info("{} re-sent to {} because {} left", next, connection.getPeerName(), disconnectedPeerName);
                        }
                    }
                }
            }
    ...
    public void cancelPings(final Connection connection, final String disconnectedPeerName) {
        ...
        while (pingIt.hasNext()) {
            ...
            if (shouldRemove) {
                log.info("Removing ping of {} since it is disconnected", pingPeerName);
                ...
                for (Pong pong : pingContext.getPongs()) {
                    ...
                    if (c != null) {
                        c.send(cancelPings);
                        log.info("{} sent to {}", cancelPings, pong.getPeerName());
                    } else {
        ...
        while (pingIt.hasNext()) {
            ...
            if (pingContext.isTimeout()) {
                ...
                if (config.getPeerName().equals(pingPeerName)) {
                    ...
                    log.info("Ping for {} has timed out. Notifying futures with # peers: {}", pingContext.getPeerName(),
                            peers.size());
                    for (CompletableFuture<Collection<String>> future : pingContext.getFutures()) {
                        future.complete(peers);
                    }
                } else {
                    log.info("Ping for {} has timed out.", pingContext.getPeerName());
                }
            }
        }
    ...
    private PingContext discoveryPing() {
        ...
        log.info("Doing a full ping with ttl={}", ttl);
