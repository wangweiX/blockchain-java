...
public class PingContext {
    ...
    public boolean handlePong(final String thisServerName, final Pong pong) {
        ...
        if (pongs.containsKey(pongServerName)) {
            log.debug("{} from {} is already handled for {}", pong, pongServerName, ping.getPeerName());
            return false;
        }

        pongs.put(pongServerName, pong);

        log.debug("Handling {} from {} for {}. Pong #: {}", pong, pongServerName, ping.getPeerName(), pongs.size());

        if (!thisServerName.equals(ping.getPeerName())) {
            if (connection != null) {
                final Pong next = pong.next(thisServerName);
                if (next != null) {
                    log.debug("Forwarding {} to {} for initiator {}", pong, connection.getPeerName(), ping.getPeerName());
                    connection.send(next);
                } else {
                    log.error("Invalid {} received from {} for {}", pong, pongServerName, ping.getPeerName());
                }
            } else {
                log.error("No connection is found in ping context for {} from {} for {}", pong, pongServerName,
                        ping.getPeerName());
            }
        }
